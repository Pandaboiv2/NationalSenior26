...
def grab_tiles(target_matrix: list, target_row: int, direction: int, grabbed_tiles: list, facing: int = 1) -> list:
    ...
    if abs(direction) > 0:
        ...
        if direction == -1:
            move_motors(-300 * facing, 300 * facing, rotations=0.28)
        elif direction == 1:
            move_motors(300 * facing, -300 * facing, rotations=0.32)
        move_motors(300 * facing, 300 * facing, rotations=0.75)
...
def two_same(color, grabbed_tiles, color_arrays):
    # go to that color
    move_to_tiles(color)

    # grab both tiles in row 0 (front pair)
    color_arrays[color - 1] = grab_tiles(color_arrays[color - 1], 0, 0, grabbed_tiles)

    # mark front-left and front-right
    grabbed_tiles[0] = color
    grabbed_tiles[1] = color
...
def two_not_same(left_color, right_color, grabbed_tiles, color_arrays):
    # decide which color to take first (closest)
    # here: smaller color number = "closer"
    if left_color <= right_color:
        first_color, first_dir = left_color, -1   # left side
        second_color, second_dir = right_color, 1 # right side
    else:
    ...
    # go back to center
    go_to_center(second_color)

    return grabbed_tiles, color_arrays

def grab_first_four_tiles(mosaic_pattern: list, grabbed_tiles: list, color_arrays: list):
    # we only care about front row now:
    # index 1 = front-left, index 5 = front-right
    left_color = mosaic_pattern[1]
    right_color = mosaic_pattern[5]

    if left_color == right_color:
        grabbed_tiles, color_arrays = two_same(left_color, grabbed_tiles, color_arrays)
    else:
        grabbed_tiles, color_arrays = two_not_same(left_color, right_color, grabbed_tiles, color_arrays)
...
def grab_horizontal(mosaic_pattern: list, grabbed_tiles: list, color_arrays: list):
    ...
    grabbed_tiles[2] = back_color  # CHANGED: moved after grab
    grabbed_tiles[3] = back_color  # CHANGED: moved after grab
...
def grab_horizontal_single(mosaic_pattern: list, grabbed_tiles: list, color_arrays: list):
    ...
    if pair_row_is_back:
        ...
        grabbed_tiles[2] = pair_color  # CHANGED: moved after grab
        grabbed_tiles[3] = pair_color  # CHANGED: moved after grab

        first_front = min(single1, single2, key=lambda c: abs(c - pair_color))
        second_front = single1 if first_front == single2 else single2

        # CHANGED: direction now based on which side the color is on, not hardcoded
        first_front_dir = -1 if first_front == front_left else 1
        second_front_dir = -1 if second_front == front_left else 1

        go_to_some_tiles(first_front, pair_color)
        color_arrays[first_front - 1] = grab_tiles(color_arrays[first_front - 1], 0, first_front_dir, grabbed_tiles)
        grabbed_tiles[1 if first_front_dir == -1 else 0] = first_front  # CHANGED: index based on dir

        go_to_some_tiles(second_front, first_front)
        color_arrays[second_front - 1] = grab_tiles(color_arrays[second_front - 1], 0, second_front_dir, grabbed_tiles)
        grabbed_tiles[1 if second_front_dir == -1 else 0] = second_front  # CHANGED: index based on dir

    else:
        first_back = min(single1, single2, key=lambda c: abs(c - pair_color))
        second_back = single1 if first_back == single2 else single2

        # CHANGED: direction based on which side the color is on
        first_back_dir = -1 if first_back == back_left else 1
        second_back_dir = -1 if second_back == back_left else 1

        move_to_tiles(first_back)
        color_arrays[first_back - 1] = grab_tiles(color_arrays[first_back - 1], 0, first_back_dir, grabbed_tiles)
        grabbed_tiles[2 if first_back_dir == -1 else 3] = first_back  # CHANGED: index based on dir

        go_to_some_tiles(second_back, first_back)
        color_arrays[second_back - 1] = grab_tiles(color_arrays[second_back - 1], 0, second_back_dir, grabbed_tiles)
        grabbed_tiles[2 if second_back_dir == -1 else 3] = second_back  # CHANGED: index based on dir
...
def grab_vertical(mosaic_pattern: list, grabbed_tiles: list, color_arrays: list):
    ...
    # front of second color — CHANGED: target_row=1
    ...
    # front of first color — CHANGED: target_row=1
...
def grab_vertical_single(grabbed_tiles, color_arrays, mosaic_colors):
    ...
    if a == b:
        first_color = a
        second_bottom_color = c
        second_top_color = d
        direction = -1
    else:
        first_color = c
        second_bottom_color = a
        second_top_color = b
        direction = 1

    # Trip 1: back of doubled color
    move_to_tiles(first_color)
    ...
    # Trip 4: front of doubled color — target_row=1
    go_to_some_tiles(first_color, second_top_color)
    color_arrays[first_color - 1] = grab_tiles(color_arrays[first_color - 1], 1, direction, grabbed_tiles)
    if direction == -1:
        grabbed_tiles[0] = first_color
    else:
        grabbed_tiles[1] = first_color
...
def grab_three_same(mosaic_pattern, grabbed_tiles, color_arrays):
    ...
    if back_same:
        ...
        # Step 3: triple color front same side — CHANGED: target_row=1
        front_dir, front_index = front_same_side
        color_arrays[triple_color - 1] = grab_tiles(color_arrays[triple_color - 1], 1, front_dir, grabbed_tiles)
        grabbed_tiles[front_index] = triple_color

        # Step 4: triple color front other side — CHANGED: target_row=1
        if front_other_side is not None:
            front_dir, front_index = front_other_side
            color_arrays[triple_color - 1] = grab_tiles(color_arrays[triple_color - 1], 1, front_dir, grabbed_tiles)  # CHANGED: target_row=1
            grabbed_tiles[front_index] = triple_color
